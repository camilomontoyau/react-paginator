...
const Paginator = (props) => {
    ...
    const [state, setState] = reactExports.useState(() => init());
    ...
    reactExports.useEffect(() => {
        ...
        setState(Object.assign(Object.assign({}, state), { totalRecords: props.totalRecords, totalPages }));
    }, [props.totalRecords]);
    const handleClick = (page, evt) => {
        evt.preventDefault();
        gotoPage(page);
    };
    const handleMoveLeft = reactExports.useCallback((evt) => {
        evt.preventDefault();
        gotoPage(props.currentPage - state.pageNeighbours * 2 - 1);
    }, [props.currentPage]);
    const handleMoveRight = reactExports.useCallback((evt) => {
        evt.preventDefault();
        gotoPage(props.currentPage + state.pageNeighbours * 2 + 1);
    }, [props.currentPage, state.pageNeighbours]);
    const fetchPageNumbers = reactExports.useCallback(() => {
    ...
    return (React.createElement("ul", { className: props.pageContainerClass }, pages.map((page, index) => {
        if (page === LEFT_PAGE)
            return (React.createElement("li", { className: props.pageItemClass, key: index },
                React.createElement("button", { className: props.pagePrevClass, onClick: handleMoveLeft }, props.pagePrevText)));
        if (page === RIGHT_PAGE) {
            return (React.createElement("li", { className: props.pageItemClass, key: index },
                React.createElement("button", { className: props.pageNextClass, onClick: handleMoveRight }, props.pageNextText)));
        }
        return (React.createElement("li", { className: `${props.pageItemClass} ${currentPage === page ? props.pageActiveClass : null}`, key: index },
            React.createElement("button", { className: props.pageLinkClass, onClick: e => handleClick(page, e) }, page)));
    })));
};
